...
public class Tests
{
    ...
    {
        Method[] methods = Arrays.stream(LOGGER_CLASS.getDeclaredMethods()).filter(m -> m.getName().equals(methodName)).filter(m -> m.getParameterCount() == argsCount).toArray(Method[]::new);
        if (methods.length == 0)
        {
